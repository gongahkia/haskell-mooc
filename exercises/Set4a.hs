...
middle :: Ord x => x -> x -> x -> x
middle val1 val2 val3 = (sort ([val1, val2, val3]) !! 1)
...
incrementKey :: (Eq x, Num y) => x -> [(x, y)] -> [(x, y)]
incrementKey key valuesList = map (\(keey, val) -> if keey == key then (keey, val + 1) else (keey, val)) (valuesList)
...
freqs :: (Eq x, Ord x) => [x] -> Map.Map x Int
freqs list = foldr (\element -> Map.alter (\maybeCurrentValue -> 
    case maybeCurrentValue of 
        { Just currentCount -> Just (currentCount + 1); Nothing -> Just (1) }) (element)) (Map.empty) (list)
...
transfer from to amount bank = 
    if Map.notMember (from) (bank) || Map.notMember (to) (bank) || amount < 0 || ((Map.findWithDefault (0) (from) (bank)) < amount) then bank 
...
swap :: Ix i => i -> i -> Array i x -> Array i x
...
maxIndex :: (Ix i, Ord x) => Array i x -> i
maxIndex array = fst (maximumBy (comparing (snd)) (assocs (array)))
