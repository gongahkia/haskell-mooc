...
(|||) :: Bool -> Bool -> Bool
argument1 ||| argument2 = if argument2 then True else argument1
...
boolLength :: [Bool] -> Int
boolLength [] = 0
boolLength (boolean : listOfBooleans) = if boolean then 1 + boolLength (listOfBooleans) else 1 + boolLength (listOfBooleans)
