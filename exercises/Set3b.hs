...
buildList start count end = if count == 0 then [end] else start : buildList (start) (count - 1) (end)
...
sums n = helper (1) (1) (n)
    where
        helper i j n = if i == n then [j] else j : helper (i + 1) (j + i + 1) (n)
...
mylast :: a -> [a] -> a
mylast defaultValue [] = defaultValue
mylast _ (currentLast : restOfTheList) = mylast (currentLast) (restOfTheList)
...
indexDefault (current : restOfTheList) i def = if i < 0 then def else if i == 0 then current else indexDefault (restOfTheList) (i - 1) (def)
...
sorted [] = True
sorted [x] = True
sorted (x : y : restOfTheList) = if x <= y then sorted (y : restOfTheList) else False
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf list = helper (0) (list)
    where
        helper _ [] = []
        helper currentSum (x : restOfTheList) = let newSum = currentSum + x in newSum : helper (newSum) (restOfTheList)
...
merge [] list2 = list2
merge list1 [] = list1
merge (x : list1) (y : list2) = if x <= y then x : merge (list1) (y : list2) else y : merge (x : list1) (list2)
...
mymaximum bigger initial (x : restOfTheList) = if bigger (x) (initial) then mymaximum (bigger) (x) (restOfTheList) else mymaximum (bigger) (initial) (restOfTheList)
...
map2 function [] _ = []
map2 function _ [] = []
map2 function (x : restOfAs) (y : restOfBs) = function (x) (y) : map2 (function) (restOfAs) (restOfBs)
...
maybeMap function [] = []
maybeMap function (current : restOfTheList) = case function (current) of
    Nothing  -> maybeMap (function) (restOfTheList)
    Just (x) -> x : maybeMap (function) (restOfTheList)
