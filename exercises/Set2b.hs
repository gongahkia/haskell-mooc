...
binomial n k = if k == 0 then 1 else if n == 0 then 0 else binomial (n - 1) (k) + binomial (n - 1) (k - 1)
...
oddFactorial n = if n == 1 then 1 else if even (n) then oddFactorial (n - 1) else n * oddFactorial (n - 2)
...
myGcd a b = if a == 0 then b else if b == 0 then a else if a > b then myGcd (a - b) (b) else myGcd (a) (b - a)
...
smallestDivisor n = helper (n) (2)
  where
    helper n k = if mod (n) (k) == 0 then k else helper (n) (k + 1)
...
isPrime n = if n < 2 then False else if smallestDivisor (n) == n then True else False
